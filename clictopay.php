...
class Clictopay extends PaymentModule
{
    ...
    public function hookPaymentOptions($params)
    {
        ...
        if (!$cart || !$this->checkCurrency($cart)) {
            return [];
        }

        $option = new PaymentOption();

        $action_text = $this->l('Pay with SMT') . ' | ' . $this->l("It's easy, simple and secure");

        $option->setLogo(Media::getMediaPath(_PS_MODULE_DIR_ . $this->name . '/views/img/payments.png'))
            ...
            ->setInputs([
                ...
                'Montant' => [
                    'name' => 'Montant',
                    'type' => 'hidden',
                    'value' => sprintf('%.3f', (float) $cart->getOrderTotal()),
                ],
                'Devise' => [
                    'name' => 'Devise',
                    'type' => 'hidden',
                ...
                'sid' => [
                    'name' => 'sid',
                    'type' => 'hidden',
                    'value' => md5((string) $this->context->cookie->id_connections),
                ],
                'affilie' => [
                    'name' => 'affilie',
                    'type' => 'hidden',
            ...
            ->setAdditionalInformation('<p>You will be redirected to SMT to pay securely.</p>');
